...
$("#avatar-btn").addEventListener("click", () => social.openPfpEditor());
...
function renderLeaderboard() {
  ...
  $("#view-leaderboard").innerHTML = `<h3 class="sec">Standings</h3>` + levelPanel() + ranked.map((u, i) => {
    ...
    return `<div class="lb-row ${isMe ? "me" : ""} ${i === 0 ? "first" : ""} ${tipOpen === u.id ? "tip-open" : ""}" data-tip-uid="${u.id}">
      <div class="lb-rank">#${i + 1}</div>
      <div class="lb-name">${i === 0 ? '<span class="lb-crown">♛</span>' : ""}${social.avatarHtml(u, 30)}<span class="xp-badge sm">${u.lvl}</span><span>${name}</span></div>
